...
/* *************************************************************
Please fill the information based on the datasheet
******************************************************************/
...
void LCD_GPIOInit(void){
    ...
    SC_RCGCGPIO |= SC_GPIOB_CGC_ENABLE;    // activate port B
    wait++;                                // wait for port activation
    wait++;                                // wait for port activation
    GPIODIR_PB |= 0xFF;                    // make PB0-7 outputs
    GPIOAFSEL_PB &= ~0xFF;                 // disable alternate functions 
    GPIODEN_PB |= 0xFF;                    // enable digital I/O on PB0-7
    //GPIOAMSEL_PB &= ~0xFF;                 // disable analog functions

    // activate control pins
    SC_RCGCGPIO |= SC_GPIOA_CGC_ENABLE;   // activate port A
    wait++;                               // wait for port activation
    wait++;                               // wait for port activation
    GPIODIR_PA |= 0xF0;                   // make PA4-7 outputs
    GPIOAFSEL_PA &= ~0xF0;                // disable alternate functions 
    GPIODEN_PA |= 0xF0;                   // enable digital I/O on PA4-7
    //GPIOAMSEL_PA &= ~0xF0;                // disable analog
...
void LCD_PrintChar(unsigned char data){
    ...
    // Return cursor to start of line if requested
    if (data == '\r') {
        LCD_SetCursor(0, cursorY);
    }
    
    // Erase last character on request
    if (data == '\b') {
      LCD_SetCursor(cursorX - 6, cursorY);
      for (i=0; i<5; i++){
        for (j=0; j<8; j++){
          LCD_DrawPixel(cursorX + i, cursorY + j, 0x0000);
        }
      }
      return;
